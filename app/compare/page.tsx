      ...
      >
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <a href="#comparison-table" className="bg-white/10 hover:bg-white/20 text-white font-bold px-6 py-3 rounded-xl transition-colors text-sm border border-white/20">
      ...
      <div className="bg-white py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-16">
            {providers.map((p) => (
              <div key={p.slug} className={`rounded-2xl border-2 p-5 ${p.rating >= 4.5 ? 'border-orange-300 bg-orange-50' : 'border-gray-100 bg-white'} hover:shadow-md transition-all`}>
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className="font-extrabold text-gray-900 text-base">{p.name}</div>
                    <div className="flex items-center gap-1 mt-0.5">
                      {'★★★★★'.slice(0, Math.round(p.rating)).split('').map((s, i) => (
                  ...
                  {p.badge && (
                    <span className={`text-xs font-bold px-2 py-1 rounded-lg ${p.rating >= 4.5 ? 'bg-orange-500 text-white' : 'bg-gray-100 text-gray-600'}`}>
                      {p.badge.replace('⭐ ', '')}
                    </span>
                  )}
                </div>
              ...
              <table className="w-full border-collapse text-sm min-w-[900px]">
                <thead>
                  <tr className="bg-gray-900">
                    <th className="text-left px-5 py-4 text-white font-bold w-36 sticky left-0 bg-gray-900">Provider</th>
                    <th className="px-4 py-4 text-white font-semibold text-center text-xs">Starting Price</th>
                    <th className="px-4 py-4 text-white font-semibold text-center text-xs">Cover Levels</th>
                    <th className="px-4 py-4 text-white font-semibold text-center text-xs">Online Quote</th>
                    {coverColumns.map((c) => (
                      <th key={c.key} className="px-4 py-4 text-white font-semibold text-center text-xs">{c.label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {providers.map((p, i) => (
                    <tr key={p.slug} className={`${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-orange-50/40 transition-colors`}>
                      <td className={`px-5 py-4 border-b border-gray-100 sticky left-0 ${i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                        <div className="font-extrabold text-gray-900 text-sm leading-tight">{p.name}</div>
                        {p.badge && p.rating >= 4.5 && (
                          <span className="text-xs text-orange-500 font-bold">⭐ Top Rated</span>
                        )}
                      </td>
                      ...
                      {coverColumns.map((c) => (
                        <td key={c.key} className="px-4 py-4 border-b border-gray-100 text-center">
                          {p[c.key]
                            ? <span className="text-green-500 font-bold text-base">✓</span>
          ...
          <div className="mb-16">
            ...
            <div className="space-y-6">
              {providers.map((p) => (
                <div key={p.slug} className={`rounded-2xl border-2 overflow-hidden ${p.rating >= 4.5 ? 'border-orange-200' : 'border-gray-100'}`}>
                  <div className={`px-6 py-4 flex flex-wrap items-center gap-4 border-b ${p.rating >= 4.5 ? 'bg-orange-50 border-orange-100' : 'bg-gray-50 border-gray-100'}`}>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 flex-wrap">
                        <h3 className="text-xl font-extrabold text-gray-900">{p.name}</h3>
                        {p.badge && <span className={`text-xs font-bold px-3 py-1 rounded-full ${p.rating >= 4.5 ? 'bg-orange-500 text-white' : 'bg-gray-200 text-gray-700'}`}>{p.badge}</span>}
                      </div>
